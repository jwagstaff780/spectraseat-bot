...
// Bulk endpoint: cheap, but only supports "core" markets (h2h, spreads,
// totals) — which markets to ask for is per-sport (see config.TRACKED_SPORTS).
async function getCoreOdds(sportKey, markets) {
  const key = requireKey();
  const url =
    `${config.ODDS_API_BASE}/sports/${sportKey}/odds/?apiKey=${key}` +
    `&regions=${config.REGIONS}&markets=${markets.join(",")}&oddsFormat=decimal`;
  return apiGet(url);
}
